...
def simulation_wellmixed_phylogeny_with_fitdiff(
    T: float,
    N: int,
    ...
    """
    Simulate the basic model with Gillespie algorithm for a well-mixed population,
    recording phylogeny with fitness differences to mean fitness at strain creation.
    ...
    Returns:
        report (pd.DataFrame): dataframe with the recorded data including fitness_difference column
    """
    if N < 2:
        raise ValueError(
    ...
    while True:
        ...
        if rng.random() > mutation_rate_total / (
            ...
            mean_fitness = np.sum(fitness.flatten() * populations) / N
            fitness_difference = fitness_new - mean_fitness

            new_strain = len(recorder.phylogeny)
            recorder.record_strain(
                t, fitness_new, None, int(target_strain), fitness_difference
            )
