...
def simulation_final_state(
    ...
    """
    Simulate the basic model descriped in the paper with Gillespie algorithm, without recording the phylogeny.
    ...
    Args:
        ...
        rng_seed (int): seed for the random number generator
    Returns:
        fitness_state (np.ndarray): final fitness state of the simulation
    """
    A = np.asarray(A, dtype=float)
    assert A.shape[0] == A.shape[1]
    ...
    fitness_state = pd.DataFrame(
        ...
        index=pd.Index(range(A.shape[0]), name="location"),
    )
